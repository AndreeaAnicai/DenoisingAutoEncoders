...
def MatrixFactorization(data: object, delimiter: object = None, useCols: object = 'all', dtype: object = None,
                        method: object = None, percTrain: object = 0.7, numFolds: object = None, log: object = True,
                        logIteration: object = False, seed: object = 100, learningRate: object = 0.005, lambdaReg: object = 0.05,
                        numFeatures: object = 10, iterations: object = 75) -> object:
    ...
    for fold in range(numFolds):
        ...
        for iteration in range(iterations):
            ...
            if log == True:
                print("")
                print("Root Mean Squared Error (RMSE) for iteration " + str(iteration + 1) + " on train set: " + str(
                    RMSEiter))
                print(
                    "Mean Absolute Error (MAE) for iteration " + str(iteration + 1) + " on train set: " + str(MAEiter))
                print("")
        ...
        if log == True:
            if method == 'cv':
                print(
                    "Fold " + str(fold + 1) + ": Root Mean Squared Error (RMSE) on train set: " + str(RMSEtrain[fold]))
    ...
    if log == True:
        if method == 'cv':
            print("Mean of Root Mean Squared Error (RMSE) on train sets (over " + str(numFolds) + "folds): " + str(
                np.mean(RMSEtrain)))
            print("Mean of Root Mean Squared Error (RMSE) on test sets (over" + str(numFolds) + " folds): " + str(
                np.mean(RMSEtest)))

            print("Mean of Mean Absolute Error (MAE) on train sets (over " + str(numFolds) + "folds): " + str(
                np.mean(MAEtrain)))
            print("Mean of Mean Absolute Error (MAE) on test sets (over " + str(numFolds) + "folds): " + + str(
                np.mean(MAEtest)))
...
if __name__ == "__main__":
    ...
    '''
    A = pd.read_csv('autoencoder_train_predict/deleted_missing_final.csv')

    # Replace nan values from array
    A = A.replace(np.nan, -99999999)
    A = A.replace(-99999999, 0)

    MatrixFactorization(A, ',', 'all', None, object, 0.7, 10, True, True, 100, 0.005, 0.05, 402, 75)
